...
See [**example_modules**](./example_modules) folder for usage examples of [validators](_media/schema.ts) and [types](_media/settings.ts).
...
`undefined` \| \{ `modulesDir`: `string`; `sections`: \{ `copy`: `boolean`; `file_name`: `string`; `prefix`: `string`; \}; `snippets`: \{ `copy`: `boolean`; `file_name`: `string`; `prefix`: `string`; \}; `themeRoot`: `string`; \}
